...
\*********************************/
	...
	if($action == '' || $action == 'home') {
		...
		//VISITADOS
		$smarty->assign("tsVisitados",$tsCom->getVisitados());
		...
		if($tsUser->is_member) {
			if(isset($_POST['nombre'])) {
				...
				if($new_com > 0) {
					$tsPage = 'aviso';
					$smarty->assign("tsAviso",array('titulo' => 'Felicitaciones!', 'mensaje' => 'El mundo entero est&aacute; ante la presencia de una nueva comunidad', 'but' => 'Ir a la comunidad', 'link' => "{$tsCore->settings['url']}/comunidades/".$tsCore->setSeo($_POST['ncorto'])."/"));
				} else $tsError = $new_com;
			}
//		} else return 'Debes ser miembro para poder crear comunidades.';
		}else{		
			$tsPage = 'aviso';
			$smarty->assign("tsAviso",array('titulo' => 'Aviso', 'mensaje' => 'Debes ser miembro para poder crear comunidades', 'but' => 'Registrarse', 'link' => "{$tsCore->settings['url']}/registro/"));	
		}
		...
		if(is_array($get_com)) {
			if($act == 'editar') {
				...
				if(isset($_GET['bid'])) {
					...
					//FIX CREAR TEMA DESDE BORRADORES  Y SE UTILIZA PARA PUBLICAR UN TEMA NUEVO DESDE NU BORRADOR
					...
					//ESTA FUNCION SE UTILIZA PARA PUBLICAR UN TEMA NUEVO
					if(isset($_POST['titulo'])) {
						$new_tema = $tsCom->nuevo_tema($get_com['com']['c_id']);
						if($new_tema > 0) {
							$tsPage = 'aviso';
