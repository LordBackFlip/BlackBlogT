...
class tsMonitor {
	...
	*/
	public function __construct(){
		...
		// NOTIFICACIONES
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT COUNT(not_id) AS total FROM u_monitor WHERE user_id = \''.$tsUser->uid.'\' AND not_type != 60 AND not_type!= 5 AND not_menubar > 0');
		$data = db_exec('fetch_assoc', $query);
		$this->notificaciones = $data['total'];
		
		//NOTIFICACIONES DE NUEVOS SHOUTS DE USUARIOS QUE SIGO BY TO-UP		
		$q = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT COUNT(not_id) AS total FROM u_monitor WHERE user_id = \''.$tsUser->uid.'\' AND not_type = 60 AND not_menubar > 0');
		$data = db_exec('fetch_assoc', $q);		
		$this->nuevo = $data['total'];
		
		//NOTIFICACIONES DE NUEVOS POSTS DE USUARIOS QUE SIGO BY TO-UP		
		$q = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT COUNT(not_id) AS total FROM u_monitor WHERE user_id = \''.$tsUser->uid.'\' AND not_type = 5 AND not_menubar > 0');
		$data = db_exec('fetch_assoc', $q);		
		$this->nuevopost = $data['total'];		
    ...
    private function makeMonitor(){
        $this->monitor = array(
			...
			//NOTIFICACIONES MI COMO TARINGA BY TO-UP.NET			
			60 => array('text' => 'public&oacute; un nuevo', 'ln_text' => 'shout', 'css' => 'blue_ball'),
			//MENCION EN COMENTARIOS DEL POST
			61 => array('text' => array('te mencion&oacute; en un', '_REP_ te mencionaron en un'), 'ln_text' => 'comentario', 'css' => 'blue_ball'),
			//MENCION EN UN SHOUT
			62 => array('text' => 'te mencion&oacute; en un', 'ln_text' => 'shout', 'css' => 'blue_ball'),
        );
    }
...
public function setNotificacion($type = NULL, $user_id = NULL, $obj_user = NULL, $obj_uno = 0, $obj_dos = 0, $obj_tres = 0){
	...
	public function getNotificaciones($unread = false){
		...
		if($this->show_type == 1) {
            ...
            if($this->notificaciones > 5 || $unread == true ){
    			// CONSULTA
    			$sql = 'SELECT m.*, u.user_name AS usuario FROM u_monitor AS m LEFT JOIN u_miembros AS u ON m.obj_user = u.user_id WHERE m.user_id = \''.$tsUser->uid.'\' AND not_type!= 60 AND not_type!= 5 AND m.not_menubar '.$not_view.' ORDER BY m.not_id DESC';
            } else {
    			// CONSULTA
    			$sql = 'SELECT m.*, u.user_name AS usuario FROM u_monitor AS m LEFT JOIN u_miembros AS u ON m.obj_user = u.user_id WHERE m.user_id = \''.$tsUser->uid.'\'  AND not_type!= 60 AND not_type!= 5 ORDER BY m.not_id DESC LIMIT 5';
            }
        ...
        // ACTUALIZAMOS
        if($this->show_type == 1) db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE u_monitor SET not_menubar = \''.(int)$not_del.'\' WHERE user_id = \''.$tsUser->uid.'\' AND not_type != 60 AND not_type!= 5 AND not_menubar > 0');
        else db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE u_monitor SET not_menubar = \'0\', not_monitor = \'0\' WHERE user_id = \''.$tsUser->uid.'\' AND not_type != 60 AND not_type!= 5 AND not_monitor = \'1\'');
	...
	*/
	private function armNotificaciones($array = NULL){
...
*/			
			...
			break;			
			case 60: 
				return 'SELECT p.pub_id, u.user_name, p.p_body FROM u_muro AS p LEFT JOIN u_miembros AS u ON p.p_user_pub = u.user_id WHERE p.pub_id = \''.(int)$data['obj_uno'].'\' LIMIT 1';	
			break;	
			case 62:
    ...
    private function makeOracion($data = NULL){
        ...
        switch($no_type){
			...
			case 61:
                ...
                // ID COMMENT
                if($no_type == 8 || $no_type == 9 || $no_type == 61){
...
*/			
			case 15:
                //$oracion['text'] = 'Recibiste una nueva <span class="qtip" title="'.$data['m_title'].'">medalla <img src="'.$site_url.'/themes/default/images/icons/med/'.$data['m_image'].'_16.png"/></span>';
				$oracion['text'] = 'Recibiste una nueva <span class="qtip" title="'.$data['m_title'].'">medalla</span>';
			break;
			case 16:
	...
	*/
	public function getFollows($type = NULL, $user_id = 0){
		...
		switch($type){
			...
			break;
			case 'siguiendo':
				$query = 'SELECT u.user_id, u.user_name, p.user_pais, p.p_mensaje, f.follow_id FROM u_miembros AS u LEFT JOIN u_perfil AS p ON u.user_id = p.user_id LEFT JOIN u_follows AS f ON p.user_id = f.f_id WHERE f.f_user = \''.(int)$user_id.'\' AND f.f_type = \'1\' AND u.user_activo = \'1\' AND u.user_baneado = \'0\' ORDER BY f.f_date DESC';
                // PAGINAR
                $total = db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', $query));
                $pages = $tsCore->getPagination($total, 12);
                $data['pages'] = $pages;
			...
			break;
            case 'posts':
				$query = 'SELECT f.f_id, p.post_user, p.post_title, u.user_name, c.c_seo, c.c_nombre, c.c_img FROM u_follows AS f LEFT JOIN p_posts AS p ON f.f_id = p.post_id LEFT JOIN u_miembros AS u ON u.user_id = p.post_user LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE f.f_user = \''.(int)$user_id.'\' AND f.f_type = \'2\' AND p.post_status = \'0\' ORDER BY f.f_date DESC';
                // PAGINAR
                $total = db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', $query));
                $pages = $tsCore->getPagination($total, 12);
                $data['pages'] = $pages;
