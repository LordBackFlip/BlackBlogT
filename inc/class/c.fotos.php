...
class tsFotos {
    ...
    */
    function getLastComments(){
        ...
        //
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT c.cid, c.c_user, f.foto_id, f.f_title, f.f_status, u.user_name, u.user_activo FROM f_comentarios AS c LEFT JOIN f_fotos AS f ON c.c_foto_id = f.foto_id LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.f_status = \'0\' && u.user_activo = \'1\' && u.user_baneado = \'0\' ORDER BY c.c_date DESC LIMIT 10');
    ...
    /*
        getFotos($user_id)
    */
    function getFotos($user_id = NULL){
        global $tsCore, $tsUser;
        //
    ...
    */
    function getFoto(){
        ...
        $fid = intval($_GET['fid']);
        // MORE FOTOS
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT f.*, u.user_seguidores, u.user_puntos, u.user_name, u.user_activo, u.user_lastactive, p.user_pais, p.user_sexo, u.user_rango, r.r_name, r.r_color, r.r_image FROM f_fotos AS f LEFT JOIN u_miembros AS u ON u.user_id = f.f_user LEFT JOIN u_perfil AS p ON p.user_id = u.user_id LEFT JOIN u_rangos AS r ON u.user_rango = r.rango_id WHERE f.foto_id = \''.(int)$fid.'\' '.($tsUser->is_admod || $tsUser->permisos['moacp'] ? '' : 'AND f.f_status = \'0\' AND u.user_activo = \'1\'').' LIMIT 1');
        ...
        if($data['foto']['user_lastactive'] > $is_online) $data['foto']['status'] = array('t' => 'Usuario Online', 'css' => 'online');
        elseif($data['foto']['user_lastactive'] > $is_inactive) $data['foto']['status'] = array('t' => 'Usuario Inactivo', 'css' => 'inactive');
        else $data['foto']['status'] = array('t' => 'Usuario Offline', 'css' => 'offline');		
