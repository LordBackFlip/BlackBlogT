...
class tsCuenta {
    ...
    /*
        loadExtras()
    */
    private function unData($data = NULL){
        //
	...
	*/
	function loadHeadInfo($user_id = NULL){
		global $tsUser, $tsCore;
		// INFORMACION GENERAL
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT u.user_id, u.user_name, u.user_registro, u.user_lastactive, u.user_activo, u.user_baneado, p.user_sexo, p.user_pais, p.p_nombre, p.p_avatar, p.p_mensaje, p.p_socials, p.p_empresa, p.p_configs, p.user_dia, p.user_mes, p.user_ano, p.p_sitio FROM u_miembros AS u, u_perfil AS p WHERE u.user_id = \''.(int)$user_id.'\' AND p.user_id = \''.(int)$user_id.'\'');
		$data = db_exec('fetch_assoc', $query);
        // INCLUIR BANDERITA EN EL PERFIL
...
//		$q5 = db_exec('fetch_row', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT COUNT(follow_id) AS s FROM u_follows WHERE f_id =\''.(int)$user_id.'\' && f_type = \'1\''));
        
        $data['stats']['user_posts'] = $q1[0];
		$data['stats']['user_seguidores'] = $q2[0];
		$data['stats']['user_comentarios'] = $q3[0];
//		$data['stats']['user_seguidores'] = $q5[0];
		...
		//NUMERO DE TEMAS
        $q5 = db_exec('fetch_row', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT COUNT(t_id) AS t FROM c_temas WHERE t_autor = \''.(int)$user_id.'\' && t_estado = \'0\''));
        $data['stats']['user_temas'] = $q5[0];	
		...
		//CALCULAR KARMA DEL USUARIO BY TO-UP.NET
		$KarmaOriginal = 2;
		$a1 = ($data['stats']['user_posts'])/100;
		$a2 = ($data['stats']['user_comentarios'])/100;
		$a3 = ($data['stats']['user_temas'])/100;		
		$a4 = ($data['stats']['user_seguidores'])/100;
		$a5 = ($data['stats']['user_temas'])/100;
		$a6 = ($data['stats']['user_puntos'])/100;
		$data['calcular']['karma'] = $KarmaOriginal+$a1+$a2+$a3+$a4+$a5+$a6;
	...
	/*
		loadGeneral($user_id)
	*/
	function loadGeneral($user_id = NULL){
		global $tsCore;
		// MEDALLAS
    ...
    */
    ...
    /*
        loadPosts($user_id)
    */
    function loadPosts($user_id = NULL){
        global $tsUser, $tsCore;
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT p.post_portada, p.post_body, p.post_id, p.post_title, p.post_puntos, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' AND p.post_user = \''.(int)$user_id.'\' ORDER BY p.post_date DESC LIMIT 15');
		...
		foreach($data['posts'] as $id => $row){		
			$data['posts'][$id]['portada'] = $tsCore->extraer_img($data['posts'][$id]['post_body']);
		}		
		//SACAR TEXTO DEL POST
		foreach($data['posts'] as $id => $row){		
			$data['posts'][$id]['contenido'] = strip_tags(substr($tsCore->LimpiarBBcode($data['posts'][$id]['post_body']), 0, 500)).'';
		}
		//		
        return $data;
    }
    ...
    */
    function loadComunidades($user_id = NULL){
	...
	/*
        loadTemas($user_id)
    */
    function loadTemas($user_id = NULL){
		global $tsCore;
		// Temas
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT t.t_id, t.t_titulo, t.t_respuestas, t.t_cuerpo, c.c_nombre, cat.c_seo, c.c_nombre_corto FROM c_temas AS t LEFT JOIN c_comunidades AS c ON t.t_comunidad = c.c_id LEFT JOIN c_categorias AS cat ON c.c_categoria = cat.cid WHERE t.t_autor = \''.(int)$user_id.'\' ORDER BY t_fecha DESC LIMIT 20');
		$data['data'] = result_array($query);	
        $data['total'] = count($data['data']);
		//
		foreach($data['data'] as $id => $row){		
			$data['data'][$id]['c_html'] = strip_tags(substr($tsCore->LimpiarBBcode($data['data'][$id]['t_cuerpo']), 0, 500)).'';
		}		
		//
        return $data;
    }	
    ...
    */
    function loadMedallas($user_id = NULL){
        //
	...
	function savePerfil(){
		...
		switch($save){
			...
			case 8: //2678400 es un mes :)
				...
				//AVISO SOLICITUD CAMBIO DE NICK
				$aviso = 'Hay una solicitud de cambio de nick del usuario: @<a href="'.$tsCore->settings['url'].'/perfil/'.$tsUser->nick.'"><b>'.$tsUser->nick.'</b></a> esperando aprobaci&oacute;n.<br/><br/>Puedes acceder al panel de gesti&oacute;n de nicks <b><a href="'.$tsCore->settings['url'].'/admin/nicks">pulsando aqu&iacute;</a></b>';
	...
	*/
	...
	/*
		getPorcentVal($array)
	*/
	function getPorcentVal($array = NULL){
		//
	...
	*/
	function getPorcentTotal($array = NULL, $total = NULL){
		//
