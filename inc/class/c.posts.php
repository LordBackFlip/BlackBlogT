...
class tsPosts {
     ...
     * @return array
     */
    public function simiPosts($q = NULL)
    {
     ...
     * @return string
     */
     public function genTags($q = NULL){
	...
	*/
	function newPost(){
		...
		if($d[0]) return '0: <br>No puedes agregar posts con el mismo contenido';	
        // VACIOS
	...
	*/
    function getLastPosts($category = NULL, $subcateg = NULL, $sticky = false){
		...
		//FILTRAR UNA CATEGORIA DEL LISTADO PRINCIPAL
		/*
		if(!$category){
			$adultos = 'AND p.post_category != \'34\'';
		}				
		*/
		//FILTRO POR PAIS
		...
		if($sticky) {
			...
			$s_order = 'p.post_id DESC';			
			//SIN FILTRO PERO SE ORDENAN POR RELEVANCIA
			if($category == ''){
				$s_order = 'p.post_comments DESC, p.post_puntos DESC, p.post_favoritos DESC, p.post_hits DESC';
			}
			//FILTRAR POST EN ASCENSO
			if($category == 'ascenso'){
				$s_order = '(p.post_date AND p.post_hits) DESC';
			}	
		...
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT r.r_name, r.r_image, r.r_color, l.user_pais, p.post_portada, p.post_hits, p.post_comments, p.post_id, p.post_hits, p.post_user, p.post_category, p.post_title, p.post_date, p.post_comments, p.post_puntos, p.post_private, p.post_sponsored, p.post_status, p.post_sticky, u.user_id, u.user_name, u.user_activo, u.user_baneado, c.c_nombre, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN u_rangos AS r ON r.rango_id = u.user_rango '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : ' && u.user_activo = \'1\' && u.user_baneado = \'0\'').' LEFT JOIN p_categorias AS c ON c.cid = p.post_category LEFT JOIN u_perfil AS l ON p.post_user = l.user_id WHERE '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? 'p.post_id > 0' : 'p.post_status = \'0\' && u.user_activo = \'1\' && u.user_baneado = \'0\'').' '.$adultos.' '.$c_where.' '.$s_where.' '.$novatos.' '.$pa_where.' GROUP BY p.post_id ORDER BY '.$s_order.' LIMIT '.$start);
	...
	*/
	function getPost(){
		...
		if(empty($postData['post_id'])) {
			...
			else return array('deleted','<b>Oops!</b> El post no existe o fue eliminado!');
		}
...
//        if($postData['post_cache'] < time()-($tsCore->settings['c_stats_cache']*60)){        
		$q1 = db_exec('fetch_row', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT COUNT(u.user_name) AS c FROM u_miembros AS u LEFT JOIN p_comentarios AS c ON u.user_id = c.c_user WHERE c.c_resp = \'0\' && c.c_post_id = \''.(int)$post_id.'\' && c.c_status = \'0\' && u.user_activo = \'1\' && u.user_baneado = \'0\''));
...
//        }
		...
		//METATAGS BY TO-UP
		$postData['tags'] = $postData['post_tags'];
		//NOMBRE DE LA IMAGEN PARA EL META
		$postData['portada'] = $postData['post_portada'];
		// DESCRIPCION PARA LOS META
		$postData['descriptcion'] = htmlspecialchars(stripslashes(strip_tags($postData['post_body'])));		
		$postData['descriptcion'] = substr($postData['descriptcion'], 0, 170) ;		
		$postData['descriptcion'] = preg_replace("[\n|\r|\n\r]", ' ', $postData['descriptcion']); 		
	   ...
	   // FECHA		
		...
		//ESTA CONDICION HACE QUE SOLO SE SUME UNA VEZ LA VISTA A CADA POST POR IP
		...
		//FUNCION PARA CONTAR EL NUEMERO DE PERSONAS QUE VOTARON EN EL POSTS
		$rank_votos = db_exec('fetch_row', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT COUNT(*) FROM p_votos WHERE tid = \''.(int)$post_id.'\''));
		$postData['votos'] = $rank_votos[0];
		// TARINGA SCORE V5 BY TO-UP				
		$puntos = $postData['post_puntos'];
		if($puntos == 0) $rank_name = "0/10";
		elseif($puntos < 5 && $puntos >= 1 ) $rank_name = "0.5/10";
		elseif($puntos <= 10 && $puntos > 5 ) $rank_name = "1/10";
		elseif($puntos <= 15 && $puntos > 10 ) $rank_name = "1.5/10";
		elseif($puntos <= 20 && $puntos > 15 ) $rank_name = "2/10";
		elseif($puntos <= 25 && $puntos > 20 ) $rank_name = "2.5/10";
		elseif($puntos <= 30 && $puntos > 25 ) $rank_name = "3/10";
		elseif($puntos <= 35 && $puntos > 30 ) $rank_name = "3.5/10";
		elseif($puntos <= 40 && $puntos > 35 ) $rank_name = "4/10";
		elseif($puntos <= 45 && $puntos > 40 ) $rank_name = "4.5/10";
		elseif($puntos <= 50 && $puntos > 45 ) $rank_name = "5/10";
		elseif($puntos <= 55 && $puntos > 50 ) $rank_name = "5.5/10";
		elseif($puntos <= 60 && $puntos > 55 ) $rank_name = "6/10";
		elseif($puntos <= 65 && $puntos > 60 ) $rank_name = "6.5/10";
		elseif($puntos <= 70 && $puntos > 65 ) $rank_name = "7/10";
		elseif($puntos <= 75 && $puntos > 70 ) $rank_name = "7.5/10";
		elseif($puntos <= 80 && $puntos > 75 ) $rank_name = "8/10";
		elseif($puntos <= 85 && $puntos > 80 ) $rank_name = "8.5/10";
		elseif($puntos <= 90 && $puntos > 85 ) $rank_name = "9/10";
		elseif($puntos > 90 && $puntos <= 95 ) $rank_name = "9.5/10";
		elseif($puntos > 95 ) $rank_name = "10/10";
		$postData['score'] = $rank_name; 	
		//
		return $postData;		
	}
	...
	/*
		getSideData($array)
	*/
	function getAutor($user_id = NULL){
	   global $tsUser, $tsCore;
        // DATOS DEL AUTOR
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT u.user_registro, u.user_id, u.user_name, u.user_rango, u.user_puntos, u.user_lastactive, u.user_last_ip, u.user_activo, u.user_baneado, p.user_pais, p.user_sexo, p.user_firma, p.p_mensaje FROM u_miembros AS u LEFT JOIN u_perfil AS p ON u.user_id = p.user_id WHERE u.user_id = \''.(int)$user_id.'\' LIMIT 1');
        $data = db_exec('fetch_assoc', $query);
		...
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT rango_id, r_name, r_color, r_image FROM u_rangos WHERE rango_id = \''.$data['user_rango'].'\' LIMIT 1');
	...
	*/
	function deletePost(){
		...
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT post_portada, post_id, post_title, post_user, post_body, post_category FROM p_posts WHERE post_id = \''.(int)$post_id.'\' AND post_user = \''.$tsUser->uid.'\'');
		$data = db_exec('fetch_assoc', $query);
		...
		if(!empty($data['post_id']) || !empty($tsUser->is_admod)){
            // SI ES MI POST LO BORRAMOS Y MANDAMOS A BORRADORES
			...
			//ELIMINAR ACTIVIDAD
			//db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM u_actividad WHERE obj_uno = \''.(int)$post_id.'\' AND (ac_type = \'1\' || ac_type = \'2\' || ac_type = \'3\' || ac_type = \'4\' || ac_type = \'5\' || ac_type = \'6\' || ac_type = \'7\')');
			//ELIMINAR NOTIFICACIONES MONITOR
			//db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM u_monitor WHERE obj_uno = \''.(int)$post_id.'\' AND (not_type = \'1\' || not_type = \'2\' || not_type = \'3\' || not_type = \'5\' || not_type = \'6\' || not_type = \'7\' || not_type = \'16\')');
		} else return '0: Lo que intentas no est&aacute; permitido.';
		
	}
	
	//FUNCION PARA ELIMINARIMAGENES DE PORTADA
    function deletePortada($file = NULL){
        $root = TS_FILES.'portadas/'.$file;
        unlink($root);
        return true;
    }	
	...
	function deleteAdminPost(){
		global $tsUser;
		//CONSULTA PARA BUSCAR EL NOMBRE DE LA IMAGEN DE PORTADA
		$none = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT post_portada FROM p_posts WHERE post_id = \''.(int)$_POST['postid'].'\' ');
		$q = db_exec('fetch_assoc', $none);		
		//
			...
			//if(db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT post_id FROM p_posts WHERE post_id = \''.(int)$_POST['postid'].'\' AND post_status = \'2\''))){
				if(db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM p_posts WHERE post_id = \''.(int)$_POST['postid'].'\'')) {
					...
					if(db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM p_comentarios WHERE c_post_id = \''.(int)$_POST['postid'].'\' ')){
						db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE `w_stats` SET `stats_posts` = stats_posts - \'1\' WHERE `stats_no` = \'1\'');
						//ELIMINAR ACTIVIDAD
						//db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM `u_actividad` WHERE `obj_uno` = \''.(int)$_POST['postid'].'\' && (`ac_type` = \'1\' || `ac_type` = \'2\' || `ac_type` = \'3\' || `ac_type` = \'4\' || `ac_type` = \'5\' || `ac_type` = \'6\' || `ac_type` = \'7\')');						
						//ELIMINAR NOTIFICACIONES MONITOR
						//db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM `u_monitor` WHERE `obj_uno` = \''.(int)$_POST['postid'].'\' && (`not_type` = \'1\' || `not_type` = \'2\' || `not_type` = \'3\' || `not_type` = \'5\' || `not_type` = \'6\' || `not_type` = \'7\' || `not_type` = \'16\')');
						//						
	...
	/*
		getRelated()
	*/
	function getRelated($tags = NULL){
		global $tsCore, $tsUser;
		// ES UN ARRAT AHORA A UNA CADENA
		if(is_array($tags)) $tags = implode(", ",$tags);
		else str_replace('-',', ',$tags);
		//
		//$query = db_exec(array(__FILE__, __LINE__), 'query', "SELECT DISTINCT p.post_portada, p.post_body, p.post_id, p.post_title, p.post_category, p.post_private, c.c_seo, c.c_img, c.c_nombre FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE MATCH (p.post_tags) AGAINST ('$tags' IN BOOLEAN MODE) AND p.post_status = 0 AND post_sticky = 0 ORDER BY rand() LIMIT 0,10");
		$query = db_exec(array(__FILE__, __LINE__), 'query', "SELECT DISTINCT p.post_portada, p.post_body, p.post_id, p.post_title, p.post_category, p.post_private, c.c_seo, c.c_img, c.c_nombre FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE MATCH (p.post_tags) AGAINST ('$tags' IN BOOLEAN MODE) AND p.post_status = 0 AND post_sticky = 0 ORDER BY p.post_id ASC LIMIT 6");
		$data = result_array($query);
        //SACAR IMAGEN DEL POST
		foreach($data as $id => $row){		
			$data[$id]['portada'] = $tsCore->extraer_img($data[$id]['post_body']);
		}		
		//
		return $data;
	}
...
//		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT cm.cid, cm.c_status, u.user_name, u.user_activo, u.user_baneado, p.post_id, p.post_title, p.post_status, c.c_seo FROM p_comentarios AS cm LEFT JOIN u_miembros AS u ON cm.c_user = u.user_id LEFT JOIN p_posts AS p ON p.post_id = cm.c_post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : 'WHERE p.post_status = \'0\' AND cm.c_status = \'0\' AND u.user_activo = \'1\' && u.user_baneado = \'0\'').' ORDER BY cid DESC LIMIT 10');
	...
	//RESPUESTAS EN COMENTARIOS DE POST
	function getRespuestas($comment_id = NULL){
		global $tsCore, $tsUser;
		//
		$start = $tsCore->setPageLimit($tsCore->settings['c_max_com']);
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT u.user_name, u.user_activo, u.user_baneado, c.* FROM u_miembros AS u LEFT JOIN p_comentarios AS c ON u.user_id = c.c_user WHERE c.c_resp = \''.(int)$comment_id.'\' '.($tsUser->is_admod ? '' : 'AND c.c_status = \'0\' AND u.user_activo = \'1\' && u.user_baneado = \'0\'').' ORDER BY c.cid');
		// COMENTARIOS TOTALES
	...
	/*
		getComentarios()
	*/
	function getComentarios($post_id = NULL){
		global $tsCore, $tsUser;
		//
		$start = $tsCore->setPageLimit($tsCore->settings['c_max_com']);
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT u.user_name, u.user_activo, u.user_baneado, c.* FROM u_miembros AS u LEFT JOIN p_comentarios AS c ON u.user_id = c.c_user WHERE c.c_post_id = \''.(int)$post_id.'\' AND c.c_resp = \'0\' '.($tsUser->is_admod ? '' : 'AND c.c_status = \'0\' AND u.user_activo = \'1\' && u.user_baneado = \'0\'').' ORDER BY c.cid LIMIT '.$start);
		// COMENTARIOS TOTALES
	...
	*/
	function newComentario(){
		global $tsCore, $tsUser, $tsActividad, $tsMonitor;
        ...
        if($data['post_user']){
            if($data['post_block_comments'] != 1 || $data['post_user'] == $tsUser->uid || $tsUser->is_admod || $tsUser->permisos['mocepc']){
				...
				if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `p_comentarios` (`c_post_id`, `c_user`, `c_date`, `c_body`, `c_ip`, `c_resp`) VALUES (\''.(int)$post_id.'\', \''.$tsUser->uid.'\', \''.$fecha.'\', \''.$comentario.'\', \''.$_SERVER['REMOTE_ADDR'].'\', \''.(int)$_POST['respuesta'].'\')')) {					
					...
					if((int)$_POST['respuesta'] > 0){
						$tsMonitor->setNotificacion(9, (int)$_POST['comm_user'], $tsUser->uid, (int)$post_id, (int)$_POST['respuesta']);
					}					
					//NOTIFICACION SI TE MENCIONAN EN EL POST						
    ...
    */
    function quoteNoti($post_id = NULL, $post_user = NULL, $cid = NULL, $comentario = NULL){
	...
	//NOTIFICACION CUANDO TE MENCIONAN
	   function menTi($post_id = NULL, $post_user = NULL, $cid = NULL, $comentario = NULL){
        global $tsCore, $tsUser, $tsMonitor;
        $ids = array();
        $total = 0;
        //
        ...
        if(!empty($users[1])) {
            foreach($users[1] as $user){
                ...
                if($user != $tsUser->nick){
                    ...
                    if(!empty($uid) && $uid != $tsUser->uid && !in_array($uid, $ids)){
                        $ids[] = $uid;
                        $tsMonitor->setNotificacion(61, $uid, $tsUser->uid, $post_id, $lcid);
                    }
                    ++$total;
                }
            }
	...
	function delComentario(){
		...
		if(!empty($is_mypost) || (!empty($is_mycmt) && !empty($tsUser->permisos['godpc'])) || !empty($tsUser->is_admod) || !empty($tsUser->permisos['moecp'])){
		    if(db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM p_comentarios WHERE cid = \''.(int)$comid.'\' AND c_user = \''.(int)$autor.'\' AND c_post_id = \''.(int)$post_id.'\'')) {
				...
				// BORRAR RESPUESTAS DE LOS COMENTARIOS BY TO-UP.NET
				db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM p_comentarios WHERE c_resp = \''.(int)$comid.'\' AND c_post_id = \''.(int)$post_id.'\'');				
	...
	*/
	function listFavoritos(){
		global $tsCore, $tsUser;
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT p.post_portada, p.post_id, p.post_title, p.post_puntos, p.post_comments, c.c_seo, c.c_img FROM p_favoritos AS f LEFT JOIN p_posts AS p ON p.post_id = f.fav_post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE f.fav_user = \''.$tsUser->uid.'\' AND p.post_status = \'0\' ORDER BY f.fav_date DESC LIMIT 7');
...
//		$data['total'] = db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT fav_id FROM p_favoritos WHERE fav_user = \''.$tsUser->uid.'\''));
	...
	*/
	...
	/*
		subirRango()
	*/
	function subirRango($user_id = NULL, $post_id = false){
		global $tsCore, $tsUser;
		// CONSULTA
	...
	/*
		DarMedalla()
	*/
	function DarMedalla($post_id = NULL){
		//
    ...
    */
    function getQuery(){
        ...
        else $search_on = 'p.post_title, p.post_body';
		// BUSQUEDA
        $trozos = explode(" ",$q); 
		$numero = count($trozos);
		if ($numero == 1) {
			$w_search = "AND p.post_title LIKE '%$q%' OR p.post_body LIKE '%$q%'";
		} elseif ($numero > 1) {		
    ...
    function getRegistrados(){
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT u.user_id, u.user_name FROM u_miembros AS u WHERE u.user_activo = \'1\' && u.user_baneado = \'0\' ORDER BY u.user_id DESC LIMIT 9');
		$data = result_array($query);
		return $data;
    }
	...
	/*
		getRelated()
	*/
	function getRelatedContenido($tags = NULL){
		global $tsCore, $tsUser;
		if(is_array($tags)) $tags = implode(", ",$tags);
		else str_replace('-',', ',$tags);
		$query = db_exec(array(__FILE__, __LINE__), 'query', "SELECT DISTINCT p.post_portada, p.post_body, p.post_id, p.post_title, p.post_category, p.post_private, c.c_seo, c.c_img, c.c_nombre FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE MATCH (p.post_body) AGAINST ('$tags' IN BOOLEAN MODE) AND p.post_status = 0 AND post_sticky = 0 ORDER BY p.post_id ASC LIMIT 10");
		$data = result_array($query);
		return $data;
	}	
