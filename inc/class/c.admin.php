...
*/
	...
	*/
	function MoveCat()
	{
		...
		if(empty($oldcid) || empty($newcid) || $oldcid < 0 || $newcid < 0)
			return 'Completa los datos.';
		elseif($oldcid == $newcid)
			return 'Selecciona una categor&iacute;a diferente.';

		if(db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE `p_posts` SET post_category = \'' . $newcid . '\' WHERE post_category = \'' . $oldcid . '\''))
			return true;
		else
			return 'Ocurri&oacute; un error, intenta de nuevo.';
	}	
    ...
    */
    ...
    setUserData
    */
    function setUserData($user_id = NULL)
    {
        global $tsCore;
        # DATA
    ...
    function deleteContent($user_id = NULL){
        global $tsUser;

		if( $user_id == 1) return "No puedes eliminar " . ($_POST['bocuenta'] ? "la cuenta" : "contenido") . " del primer Administrador hijo de puta";
    ...
    */
    function setUserRango($user_id = NULL)
    {
        global $tsUser;
    ...
    function setUserFirma($user_id = NULL)
    {
        global $tsCore;
    ...
    function ChangeNick_o_no()
    {
        ...
        die('1: <div class="box_cuerpo" style="width: 300px;">Hemos enviado un correo a <b>' .
            $datos['user_email'] .
    ...
    function DelFoto(){
        ...
        if (db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT foto_id FROM `f_fotos` WHERE foto_id = \''.(int)$foto.'\''))){			
