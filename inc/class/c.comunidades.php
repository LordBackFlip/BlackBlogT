...
class tsCom {
	...
	function comExiste($nombre = NULL) {
	...
	function es_miembro($comid = NULL) {
		global $tsUser;
	...
	function verify_permisos($comid = NULL) {
		global $tsUser;
	...
	function my_permiso($comid = NULL) {
		global $tsUser;
		$dato = db_exec('fetch_assoc', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT m_permisos FROM c_miembros WHERE m_comunidad = \''.(int)$comid.'\' AND m_user = \''.$tsUser->uid.'\' AND m_permisos > \'0\' LIMIT 1'));
	...
	function es_seguidor_com($comid = NULL) {
		global $tsUser;
	...
	function es_seguidor_tema($temaid = NULL) {
		global $tsUser;
	...
	function uploadImagen($file = NULL, $comid = NULL, $type = 1) {
		global $tsCore;		
	...
	function validImagen($file = NULL){
		// Permitido
		$max = 2097152;
	...
	function quoteNoti($temaid = NULL, $t_user = NULL, $cid = NULL, $comentario = NULL){
	...
	function mod_history($h_for = NULL, $h_type = NULL, $h_comid = NULL, $h_razon = '-') {
		global $tsCore, $tsUser;
		...
		// Solo es moderacion si el autor es diferente del mod
		if($autor != $tsUser->uid) {
	...
	function is_banned($userid = NULL, $comid = NULL) {
	...
	function getSubcats($cat_id = NULL) { 
	...
	function nueva_comunidad() {
		...
		if($tsUser->is_member) {
			...
			if(!empty($corto['c_id']) || $dato['ncorto'] == 'mis-comunidades' || $dato['ncorto'] == 'mod-history' || $dato['buscar'] || $dato['favoritos'] || $dato['borradores']) return 'El nombre corto esta en uso o no disponible, prueba con otro.';
			// Validamos IP
		...
		if(!empty($data['c_id'])) {
			if(isset($_POST['susp'])) {
				if($tsUser->is_admod) {
					...
					return '1: La comunidad <b>'.$data['c_nombre'].'</b> <br>ha sido '.($data["c_estado"] == 0 ? 'suspendida' : 'reactivada').'';
				...
				if($data['c_autor'] == $tsUser->uid || $tsUser->is_admod == 1) {
					...
					// Listo
					return '1: La comunidad <b>'.$data['c_nombre'].'</b> <br>ha sido eliminada permanentemente';
				}
			}
	...
	// Obtener comunidad
	function get_comunidad($nombre = NULL) {
		global $tsCore, $tsUser;
		// Existe la comunidad?
		...
		if($comid) {
			...
			// Ultimos miembros
			$q6 = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT m_user, user_name FROM c_miembros LEFT JOIN u_miembros ON user_id = m_user WHERE m_comunidad = \''.(int)$comid.'\' AND m_permisos > 0 ORDER BY m_fecha DESC LIMIT 21');
	...
	function getComTemas($comid = NULL) {
		global $tsCore, $tsUser;
		// Obtenemos variables
		...
		// Temas
		$q3 = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT t.t_estado, t.t_id, t.t_titulo, t.t_autor, t.t_fecha, t.t_respuestas, t.t_votos_pos, t.t_votos_neg, c.c_nombre_corto, user_name FROM c_temas AS t LEFT JOIN c_comunidades AS c ON c.c_id = t.t_comunidad LEFT JOIN u_miembros ON user_id = t.t_autor WHERE t.t_comunidad = \''.(int)$comid.'\' '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : ' AND t.t_estado = \'0\'').' AND t.t_sticky = \'0\' ORDER BY t.t_fecha DESC LIMIT '.$start.','.$max);
		$data['data'] = result_array($q3);
		// Stickys
		$q4 = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT t.t_id, t.t_titulo, t.t_autor, t.t_fecha, t.t_respuestas, t.t_votos_pos, t.t_votos_neg, c.c_nombre_corto, user_name FROM c_temas AS t LEFT JOIN c_comunidades AS c ON c.c_id = t.t_comunidad LEFT JOIN u_miembros ON user_id = t.t_autor WHERE t.t_comunidad = \''.(int)$comid.'\' '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : ' AND t.t_estado = \'0\'').' AND t.t_sticky = \'1\' ORDER BY t.t_fecha DESC');
		$data['sticky'] = result_array($q4);
		return $data;			
		
	}
	
	// Obtener datos de comunidad para editar
	function datosCom($comid = NULL) {
		global $tsCore, $tsUser;
		// Existe la comunidad?
		if(!empty($comid)) {
			// Puedo editarla o tengo permisos?
	...
	function getStats(){
		...
		$q3 = db_exec('fetch_row', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT COUNT(r_id) AS total FROM c_respuestas LEFT JOIN c_temas ON t_id = r_tema LEFT JOIN c_comunidades ON c_id = t_id WHERE r_estado = \'0\' AND t_estado = \'0\' AND c_estado = \'0\''));
		//
	...
	function getComensRecent() {
		...
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT r.r_estado, c.c_estado, t.t_estado, r.r_estado, r.r_id, t.t_id, t.t_titulo, c.c_nombre_corto, u.user_name FROM c_respuestas AS r LEFT JOIN c_temas AS t ON t.t_id = r.r_tema LEFT JOIN c_comunidades AS c ON c.c_id = t.t_comunidad LEFT JOIN u_miembros AS u ON user_id = r.r_autor WHERE u.user_activo = \'1\' && u.user_baneado = \'0\' && c.c_estado = \'0\' && r.r_estado = \'0\' && t.t_estado = \'0\' ORDER BY r.r_fecha DESC LIMIT 10');
		$array = result_array($query);
		return $array;
	}
	...
	function getPopulares(){
		...
		//
		if($tipo){
			$data = $this->getHomeTopCom($tsTops->setTime($tipo));
		}else{
			$data = $this->getHomeTopCom($tsTops->setTime(5));//TOP HISTORICO
		}
		//		
		return $data;
	}
	...
	// Comunidades populares en la home
	function getHomeTopCom($date = NULL){
		global $tsCore, $tsUser;
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT c.c_estado, c.c_id, c.c_nombre, c.c_nombre_corto, c.c_miembros, c.c_temas, c.c_fecha FROM c_comunidades AS c LEFT JOIN u_miembros AS u ON u.user_id = c.c_autor WHERE '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : 'u.user_activo = \'1\' && u.user_baneado = \'0\' && c.c_estado = \'0\' AND').' c.c_fecha BETWEEN \''.$date['start'].'\' AND \''.$date['end'].'\' ORDER BY c.c_miembros DESC LIMIT 13');
		$data = result_array($query);
		return $data;
	}
	...
	function getVisitados(){
		...
		//
		if($tipo){
			$data = $this->getHomeTopVisitados($tsTops->setTime($tipo));
		}else{
			$data = $this->getHomeTopVisitados($tsTops->setTime(5));//TOP HISTORICO
		}
		//		
		return $data;
	}
	...
	function getHomeTopVisitados($date = NULL){
		global $tsCore, $tsUser;
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT t.t_id, t.t_titulo, t.t_visitas, c.c_nombre_corto FROM c_temas AS t LEFT JOIN u_miembros AS u ON u.user_id = t.t_autor LEFT JOIN c_comunidades AS c ON c.c_id = t.t_comunidad WHERE t.t_estado = \'0\' AND t.t_fecha BETWEEN \''.$date['start'].'\' AND \''.$date['end'].'\' ORDER BY t.t_visitas DESC LIMIT 15');
		$data = result_array($query);
		return $data;
	}	
...
//						db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE c_stats SET stats_respuestas = stats_respuestas+1');
	...
	// Visitas en el tema
	function setVisitaTema($temaid = NULL) {
		global $tsUser, $tsCore;
	...
	function getMiembros($comid = NULL, $type = 'miembros') {
		global $tsCore, $tsUser;
		switch($type) {
			case 'miembros':
				// Total
		...
		if($tema['total'] > 0) {
			if($tema['t_estado'] == 0) {
				...
				if($this->es_seguidor_tema($temaid)) {
					...
					// Es mi propio tema?
					if($tema['t_autor'] == $tsUser->uid) return '0: No puedes seguir tu propio tema.';
	...
	function del_tema() {
		...
		if($tema['total'] != 0) {
			...
			if($tema['t_autor'] == $tsUser->uid) {
				...
				db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM c_favoritos WHERE f_tema = \''.(int)$temaid.'\'');
				db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM c_borradores WHERE b_tema = \''.(int)$temaid.'\'');
				db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM c_votos WHERE v_for = \''.(int)$temaid.'\'');
				db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM c_respuestas WHERE r_tema = \''.(int)$temaid.'\'');
				db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM c_temas WHERE t_id = \''.(int)$temaid.'\' LIMIT 1');
				//FIX DISMINUIR CONTADOR DE TEMAS DE LA COMUNIDAD
				db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE c_comunidades SET c_temas = c_temas-1 WHERE c_id = \''.(int)$tema['t_comunidad'].'\'');
	...
	// Buscador por tipo
    function getQueryTipo($tipo = NULL) {
        global $tsCore, $tsUser;
        //
        $q = $tsCore->setSecure($_GET['q']);
        $c = intval($_GET['cat']);
