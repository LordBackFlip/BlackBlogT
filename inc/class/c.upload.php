...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    function SubirPortada($type = 1){
        $this->type = $type;
		if($this->type == 3){
            if(empty($this->file_url)) {
                foreach($_FILES as $file)
                    $fReturn = $this->uploadFilePortada($file);
                    if(empty($fReturn['msg'])) return array('error' => $fReturn[1]);
            } else {
                $file = array(
                    'name' => substr($this->file_url, -4),
                ...
                //
                $fReturn = $this->uploadFilePortada($file, 'url');
                if(empty($fReturn['msg'])) return array('error' => $fReturn[1]);
            }
        }
        //
    ...
    function uploadFilePortada($file, $type = 'file'){
        // VALIDAR
        $error = $this->validFile($file, $type);
        if(!empty($error)){
            return array(0, $error);
			...
			$key3 = rand(0,1000);
			$key4 = rand(0,1000);
            $newName = 'toup'.$key.'a'.$key2.'b'.$key3.'c'.$key4.'d.'.$ext;
			*/
			$newName = 'toup'.$key.'.'.$ext;					
            // IMAGEN
            if($this->type == 1)
                return array(1, $this->sendFilePortada($file,$newName), $type[1]);
            // CROP
            else
				return array('msg' => $this->createImagePortada($file,$newName), 'error' => '', 'key' => $key, 'ext' => $ext);
            //
        }
    }	
	

    function sendFilePortada($file, $name){
        $url = $this->createImagePortada($file,$name);
        // SUBIMOS...
        $new_img = $this->getImagenUrl($this->uploadImagen($this->setParams($url)));
        // BORRAR
        $this->deletePortada($name);
        // REGRESAMOS
        return $new_img;
    }	
	...
	function createImagePortada($file, $name){	
        global $tsCore;
        // TAMA�O
        $size = empty($this->file_size) ? getimagesize($file['tmp_name']) : $this->file_size;
        ...
        $width = $size[0];
        $height = $size[1];
        // ESCALAR SOLO SI LA IMAGEN EXEDE EL TAMANO Y SE DEBE ESCALAR
        if($this->image_scale == true && ($width > $this->image_size['w'] || $height > $this->image_size['h'])){
                // OBTENEMOS ESCALA
                if($width > $height){
                    $_height = ($height * $this->image_size['w']) / $width;
        		...
        		imagecopyresampled($newimg, $img, 0, 0, 0, 0, $_width, $_height, $width, $height);
    			// COPIAMOS
				$root = TS_FILES.'portadas/'.$name;
                //
    			imagejpeg($newimg,$root,100);
    			imagedestroy($newimg);
    			imagedestroy($img);
                // RETORNAMOS
				return $tsCore->settings['url'].'/files/portadas/'.$name;
        } else {
            // MANTENEMOS LAS DIMENCIONES Y SOLO COPIAMOS LA IMAGEN
			return $this->copyFilePortada($file, $name);
        }
	}	
	
	
    function copyFilePortada($file,$name){
        global $tsCore;
        // COPIAMOS
        $root = TS_FILES.'portadas/'.$name;
        copy($file['tmp_name'],$root);
    ...
    public function cropAvatarPortada($key){
		$source = TS_FILES.'portadas/toup'.$_POST['key'].'.'.$_POST['ext'];	
		$size = getimagesize($source);
        // COORDENADAS
        $x = $_POST['x'];
        $y = $_POST['y'];
        ...
        imagecopyresampled($thumb, $img, 0, 0, $x, $y, $_tw, $_th, $w, $h);
		// GUARDAMOS...		
        $root = TS_FILES.'portadas/toup'.$key.'P';			
		//IMAGEN 240X180
		imagejpeg($avatar,$root.'240X180.jpg',90);
		//IMAGEN 90X70
		imagejpeg($thumb,$root.'90X70.jpg',90);
        // CLEAR
    	imagedestroy($img);
    	imagedestroy($avatar);    	
		imagedestroy($thumb);
