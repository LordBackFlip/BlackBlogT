...
<div id="res" class="boxgris" style="position:relative">
	...
	{/if}
	{if $tsError}
		<div class="aviso-error" style="margin: 20px 20px 0 20px;">{$tsError}</div>
	{/if}
	{if $tsAct == ''}
                                <div>
                                <h3 style="margin:20px 0 15px 20px">Rangos Especiales</h3>
                                <table cellpadding="0" cellspacing="0" border="0" class="admin_table" width="400" align="center">
									...
									<td colspan="6" style="text-align:right;padding: 10px;">
										<input type="button" onclick="location.href = '?act=nuevo&t=s'" class="boton azul" value="Crear Rango">		
										<!--<a href="?act=nuevo&t=s" >Agregar nuevo rango &raquo;</a>-->
									</td>
                                    </tfoot>
                                </table>
                                </div>
                                <div>
                                <h3 style="margin: 30px 0 20px 20px;">Rangos basados en el conteo de puntos y posts</h3>
                                <table cellpadding="0" cellspacing="0" border="0" class="admin_table" width="550" align="center">
									...
									<td colspan="8" style="text-align:right;padding: 10px;">
										<input type="button" onclick="location.href='?act=nuevo'" class="boton azul" value="Crear Rango">		
									</td>									
	...
	{/literal}
		<form action="" method="post">
			<div class="boxblo">
				<h2 class="mixtu">Nueva rango</h2>
				<input type="button" id="b1" onclick="$('#tab1').show(); $('#tab2').hide(); $(this).css('border','3px solid #ff6600'),$(this).css('color','#ff6600'); $('#b2').css('border','3px solid #c6c6c6'), $('#b2').css('color','#c6c6c6');" value="B&aacute;sico" style="margin-right: 18px;float: left;width:325px;border: 3px solid #ff6600;color:#ff6600;" class="boton raya"/> 				
				<input type="button" id="b2" onclick="$('#tab2').show(); $('#tab1').hide(); $(this).css('border','3px solid #ff6600'),$(this).css('color','#ff6600'); $('#b1').css('border','3px solid #c6c6c6'), $('#b1').css('color','#c6c6c6');" value="Permisos" style="float: left;width:325px;" class="boton raya"/> 
			</div>	
			<div id="tab1">			
				<div class="boxblo">				
					<div class="desbox">
						<label for="rName">T&iacute;tulo:</label>				
					</div>		
					<input type="text" id="rName" name="rName" value="{$tsRango.r_name}"/>
				</div>
				<div class="boxblo">		
					<div id="colores" style="right: 20px;">
						<span class="title">Colores</span><a href="#" id="cerrar"><img src="{$tsConfig.images}/borrar.png" /></a>
					</div>			
					<div class="desbox">					
						<label for="rColor">Color:</label>	
						<div class="com_edit_color" id="TColor" style="background: {$tsRango.r_color};" title="Color actual"></div>						
					</div>							
					<input type="text" id="rColor" name="rColor" value="{$tsRango.r_color}" style="color:#{$tsRango.r_color}; font-weight:bold;"/>
				</div>
				<div class="boxblo">
					<div class="desbox">
						<label for="gopfd">Puntos por d&iacute;a:</label>
						<span class="foquito" onclick="mydialog.alert('Información','Puntos que puede otorgar este rango a otros usuarios al d&iacute;a')"></span>
					</div>
					<input type="text" id="gopfd" name="global-pointsforday" value="{$tsRango.permisos.gopfd}"/>
				</div>		
				<div class="boxblo">
					<div class="desbox">
						<label for="gopfp">Puntos por post:</label>
						<span class="foquito" onclick="mydialog.alert('Información','Puntos que puede dar en cada post')"></span>
					</div>
					<input type="text" id="gopfp" name="global-pointsforposts" value="{$tsRango.permisos.gopfp}"/>
				</div>						
				<div class="boxblo">
					<div class="desbox">
						<label for="goaf">Anti-flood</label>
						<span class="foquito" onclick="mydialog.alert('Información','Tiempo que deben esperar entre acci&oacute;n')"></span>
					</div>
					<input type="text" id="goaf" name="global-antiflood" value="{$tsRango.permisos.goaf}"/>
				</div>
				
				<div class="boxblo">
					<div class="desbox">
						<label for="cat_img">Icono del rango:</label>						
					...
					<select name="r_img" id="cat_img">
						{foreach from=$tsIcons key=i item=img}
							<option value="{$img}" style="background:url({$tsConfig.url}/themes/default/images/icons/ran/{$img}) no-repeat left center;" {if $tsRango.r_image == $img}selected="selected"{/if}>{$img}</option>
						{/foreach}
					</select>   					
				</div>					
				<div class="boxblo">
					<h2 class="mixtu">Condici&oacute;n especial:</h2>
					<div class="boes" style="margin: 0 0 20px 0;">Cantidad requerida para obtener el rango. Elija especial si s&oacute;lo es asignado por un administrador.</div>
					<div id="nulis">
						<label onclick="$('#gocpr').show();"><input name="global-type" type="radio" id="ai_type" value="1" {if $tsRango.r_type == 1}checked{/if} class="radio"/><span class="qtip" title="Del usuario">Puntos</<span></label>
                        <label onclick="$('#gocpr').show();"><input name="global-type" type="radio" id="ay_type" value="2" {if $tsRango.r_type == 2}checked{/if} class="radio"/>Posts</label>
                        <label onclick="$('#gocpr').show();"><input name="global-type" type="radio" id="ay_type" value="3" {if $tsRango.r_type == 3}checked{/if} class="radio"/>Fotos</label>
						<label onclick="$('#gocpr').show();"><input name="global-type" type="radio" id="ay_type" value="4" {if $tsRango.r_type == 4}checked{/if} class="radio"/><span class="qtip" title="De posts y fotos">Comentarios</<span></label>
						<label onclick="$('#gocpr').hide();"><input name="global-type" type="radio" id="ay_type" value="0" {if $tsRango.r_type == 0}checked{/if} class="radio"/>Especial</label>								
						<input type="text" id="gocpr" name="global-cantidadrequerida" style="margin-top: 15px;width: 100%;{if $tsRango.r_type == 0};display:none;{/if}" maxlength="5" value="{$tsRango.r_cant}" placeholder="Especifica la cantidad que debe tener para ganarse el rango"/>											
					</div>
				</div>	
				<div class="botoboc">
					<input type="button" onclick="$('#tab2').show(); $('#tab1').hide(); $('#b2').css('border','3px solid #ff6600'), $('#b2').css('color','#ff6600'); $('#b1').css('border','3px solid #c6c6c6'), $('#b1').css('color','#c6c6c6');" value="Continuar" class="boton azul"/>
				</div>				
			</div>
