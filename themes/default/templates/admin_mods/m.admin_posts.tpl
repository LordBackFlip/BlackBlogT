								...
								<table cellpadding="0" cellspacing="0" border="0" class="admin_table" width="100%" align="center">
									<thead>
										...
										<th>Acciones <a id="actionsee" onclick="$('#actionsee').slideUp( 120, 'easeInOutElastic'); $('.right').fadeOut('slow').css('width', '920px').slideDown( 1700, 'easeInOutElastic'); $('.left').slideUp( 1500, 'easeInOutElastic'); $('#moreinfo').slideDown('fast'); "><img src="{$tsConfig.tema.t_url}/images/icons/details.png" width="14px" height="14px" title="M&aacute;s informaci&oacute;n" /></a></th>
									</thead>
									<tbody>
										{foreach from=$tsAdminPosts.data item=p}
										<tr id="post_{$p.post_id}">
											<td>{$p.post_id}</td>
											<td><a href="{$tsConfig.url}/posts/{$p.c_seo}/{$p.post_id}/{$p.post_title|seo}.html" target="_blank" class="qtip" title="{$p.post_title}">{$p.post_title|limit:10}</a></td>
											<td><a href="{$tsConfig.url}/perfil/{$p.user_name}" class="hovercard" uid="{$p.user_id}">{$p.user_name|limit:10}</a></td>
											...
											<td class="admin_actions">
												<a href="{$tsConfig.url}/posts/editar/{$p.post_id}"><img src="{$tsConfig.tema.t_url}/images/icons/editar.png" title="Editar Post" /></a>
												{if $p.post_status == 2}
													<a href="#" onclick="admin.posts.borrar({$p.post_id}); return false"><img src="{$tsConfig.tema.t_url}/images/icons/close.png" title="Borrar Post permanentemente" /></a>
												{else}
													<a href="#" onclick="mod.posts.borrar({$p.post_id}, 'posts', null); return false;"><img src="{$tsConfig.tema.t_url}/images/icons/close.png" title="Borrar Post" /></a>
												{/if}
											</td>
										</tr>
										{/foreach}
