{if $tsTemas.sticky || $tsTemas.data}	
				...
				{foreach from=$tsTemas.sticky item=t}
					<div class="com_tema_list clearfix">
						...
						<div class="ctl_info">
							...
							<div class="ctli_comment"><i class="com_icon icon_comment"></i><span>{$t.t_respuestas}</span></div>
							<div class="ctli_like"><i class="com_icon icon_hand_up"></i><span>{$t.t_votos_pos-$t.t_votos_neg}</span></div>
						</div>
					</div>
				{/foreach}
			</div>
	...
	*}
	{if $tsTemas.data}
	<div class="sepat {if $tsTemas.sticky}mejol{/if}">
		...
		{if $tsTemas.pages.pages > 1}
			<div class="com_pagination">
				{if $tsTemas.pages.prev}
					<a class="cp_prev" href="javascript:com.pages_temas({$tsTemas.pages.prev});"></a>
				{/if}
				...
				{else}
					...
					{/if}
				{/if}
				{if $tsTemas.pages.pages > 1 && $tsTemas.pages.pages > $tsTemas.pages.current}
					<a class="cp_next" href="javascript:com.pages_temas({$tsTemas.pages.next});"></a>
				{/if}
			</div>
		{/if}
	</div>
	{/if}
{else}
	<div class="nada-gris" style="border: solid #ccc;border-width: 0px 1px 1px 1px;">No se han creado temas en esta comunidad</div>
{/if}