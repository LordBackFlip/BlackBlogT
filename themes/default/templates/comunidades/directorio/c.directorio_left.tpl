<div class="com_members_filter clearfix">
	<div class="clearfix floatL">
		<form action="{$tsConfig.url}/comunidades/buscar/?q=" class="utere">
			<input type="text" class="input_text tanm" name="q" onkeypress="ibuscador_intro(event)" value="" placeholder="Buscar en comunidades">
			<input type="submit" value="Buscar" class="boto verde uilr">
		</form>	
    </div>
</div>
...
<div class="com_dir_result clearfix">
	{if $tsDir.c_seo}
    	{foreach from=$tsDir.data item=d}
			<div class="cdr_item" style="height:28px;">
				<div id="nuye">			
    ...
    {elseif $tsDir.sub.sid > 0}
    	<div id="com_members_result">
            ...
            <div style="margin-top: 10px;" class="clearfix">
            	{if $tsDir.pages.prev}
					<a class="floatL" href="{$tsConfig.url}/comunidades/dir/{$tsDir.global_url}/{$tsDir.sub.c_seo}/{$tsDir.sub.s_seo}/{$tsDir.pages.prev}">&laquo; Anterior</a>
				{/if}
                {if $tsDir.pages.pages > 1 && $tsDir.pages.pages > $tsDir.pages.current}
					<a class="floatR" href="{$tsConfig.url}/comunidades/dir/{$tsDir.global_url}/{$tsDir.sub.c_seo}/{$tsDir.sub.s_seo}/{$tsDir.pages.next}">Siguiente &raquo;</a>
				{/if}
            </div>
            {/if}
        </div>
