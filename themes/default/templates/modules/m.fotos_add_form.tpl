		...
		<form name="add_foto" method="post" action="" enctype="multipart/form-data" id="foto_form" class="form-add-post" autocomplete="off">
			...
			<div class="fade_out">
				...
				<div class="end-form clearbeta">
					<input type="button" style="width: auto; margin-left: 5px;" class="boto verde" name="new" value="{if $tsAction == 'agregar'}Agregar{else}Guardar cambios{/if}" onclick="fotos.agregar()"/>
				</div>
			</div>                    
		</form>
	</div>
{else}
	<div class="nada-amarillo">Lo sentimos, pero no puedes {if $tsAction == 'agregar'}agregar{else}editar{/if} una nueva foto</div>
{/if}
