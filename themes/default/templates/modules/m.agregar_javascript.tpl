...
var button_title = '     {if $tsDraft}Publicar{else}Publicar{/if}     ';
// {literal}
...
function save_borrador(){
	var params = 'titulo=' + encodeURIComponent($('input[name="titulo"]').val()) + '&portada=' + encodeURIComponent($('input[name="portada"]').val()) + '&cuerpo=' + encodeURIComponent($('textarea[name="cuerpo"]').val()) + '&tags=' + encodeURIComponent($('input[name="tags"]').val()) + '&categoria=' + encodeURIComponent($('select[name="categoria"]').val());
...
function borrador_save_enabled(){
	if($('#borrador-save'))
		$('#borrador-save').removeClass('disabled').removeAttr('disabled');
	borrador_is_enabled = true;
}
function borrador_save_disabled(){
	if($('#borrador-save'))
		$('#borrador-save').addClass('disabled').attr('disabled', 'disabled');
...
*/			
			...
			mydialog.show(true);
			mydialog.title('...');
			mydialog.body('<br><br>Cargando vista previa....<br><br><img src="' + global_data.url + '/themes/default/images/loading_bar.gif"><br><br>');
            mydialog.center();
            // PREVIEW
			$.ajax({
				type: 'post',
				...
				success: function(r) {
					mydialog.body(r);
					mydialog.buttons(true, true, button_title, 'postSave()', true, true, true, 'Cerrar', 'close', true, false);
					mydialog.center();
		...
		if (cuerpo.length < 1) {
			...
			$.ajax({
				...
				success: function(r) {
					...
					mydialog.center();
					$('#mydialog #buttons .boto').addClass('azul');
					//$('#mydialog #buttons .mBtn.btnOk').removeClass('btnCancel').addClass('btnGreen');
					$.scrollTo(0, 500)
				}
			});	
...
function postSave() {
	...
	mydialog.body('<div class="carf"><p>Comprobando contenido</p><img src="' + global_data.img + '/images/loading_bar.gif"/></div>');
	mydialog.buttons(false);
	mydialog.center();
