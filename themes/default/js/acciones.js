...
/* Eliminar Respuesta */
function borrar_respuesta(comid, autor, postid, gew){
	mydialog.close();
    if(!postid) var postid = gget('postid');
	if(!gew){
            mydialog.show();			
            mydialog.title('Borrar Respuesta');			
            mydialog.body('&#191;Quiere eliminar esta respuesta?');			
            mydialog.buttons(true, true, 'S&iacute;', 'borrar_respuesta(' + comid + ', ' + autor + ', ' + postid + ', 1)', true, false, true, 'No', 'close', true, true);			
            mydialog.center();			
        }else{
        $('#loading').fadeIn(250);
	$.ajax({
		...
		error: function(){
			mydialog.error_500("borrar_respuesta('"+comid+"')");
            $('#loading').fadeOut(350);
		}
	});
	}
...
function borrar_post(aceptar){
	if(!aceptar){
			...
			mydialog.buttons(true, true, 'Aceptar', 'borrar_post(1)', true, false, true, 'Cancelar', 'close', true, true);
			mydialog.center();
			return;
	}else if(aceptar==1){
			mydialog.show();
			...
			mydialog.buttons(true, true, 'Aceptar', 'borrar_post(2)', true, false, true, 'Cancelar', 'close', true, true);
			mydialog.center();
			return;
	}
...
function votar_post(puntos){
	...
	$.ajax({
		...
		success: function(h){
			...
			switch(h.charAt(0)){
				case '0': //Error
					$('#malecho').addClass('nada-rojo').html(h.substring(3)).show();
					setTimeout(function(){
						$('#malecho').hide();
					}, 2000);
				break;
		...
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
					...
					break;
				case '1': //OK
					$('#malecho').addClass('nada-verde').html(h.substring(3)).show();
					$('#cfavori').html(number_format(parseInt($('#cfavori').html().replace(".", "")) + 1, 0, ',', '.'));
					setTimeout(function(){
						$('#malecho').hide();
					}, 2000);					
					break;
...
//BOTONES COMENTARIOS SHOUTS
mySettings_cmt_shout = {
...
mySettings_cmt = {
	...
	markupSet: [
		...
		//{name:lang['Citar'], beforeInsert:function(h){ markit_quote(h); }},
		//to-up		
		{name:lang['Insertar Emoticon'], beforeInsert:function(h){ copy_toup(h); }}
	]
};

function copy_toup(h){
	$('#emoticons').slideToggle(0);
}
...
var upload = {         
	...
	agregar: function(img){                 
		// AGREGAR                 
		$.markItUp({ openWith:"[img]", closeWith: "[/img]", replaceWith: img });         
	} 
}
...
function print_editor(){
	...
	//Editor de posts comentarios
	if($('.textarea-edit') && !$('#markItUpbody_comm').length){
		$('.textarea-edit').markItUp(mySettings_cmt);
	}		
	//Editor de respuestas shouts
	if($('#nulsseea textarea') && !$('#markItUpbody_comm').length){
		$('#nulsseea textarea').markItUp(mySettings_cmt_shout);
	}	
	//Editor de respuestas shouts
...
show: function(class_aux){
	...
	if(this.close_button)
		$('#mydialog #dialog').append('<img class="close_dialog" src="'+ global_data.img +'i/ce.png" onclick="mydialog.close()" />');
	else
...
buttons: function(display_all, btn1_display, btn1_val, btn1_action, btn1_enabled, btn1_focus, btn2_display, btn2_val, btn2_action, btn2_enabled, btn2_focus){
	...
	var html = '';
	if(btn1_display)
		html += '<input type="button" class="boto verde'+(btn1_enabled?'':' disabled')+'" style="width:auto!important;display:'+(btn1_display?'inline-block':'none')+'"'+(btn1_display?' value="'+btn1_val+'"':'')+(btn1_display?' onclick="'+btn1_action+'"':'')+(btn1_enabled?'':' disabled')+' />';
	if(btn2_display)
		html += ' <input type="button" class="boto gris'+(btn1_enabled?'':' disabled')+'" style="width:auto!important;display:'+(btn2_display?'inline-block':'none')+'"'+(btn2_display?' value="'+btn2_val+'"':'')+(btn2_display?' onclick="'+btn2_action+'"':'')+(btn2_enabled?'':' disabled')+' />';
...
$(document).ready(function(){
	...
	$('body').click(function(e){ 
		...
		if ($('#fav_list').css('display') != 'none' && $(e.target).closest('#fav_list').length == 0 && $(e.target).closest('a[name=Favoritos]').length == 0) favorito.last();	   
		if ($('#shout_list').css('display') != 'none' && $(e.target).closest('#shout_list').length == 0 && $(e.target).closest('a[name=Shout]').length == 0) shout.last();
	...
	$('.userInfoLogin span.shout, .comOfi, .post-compartir img, div.action > div.btn_follow > a[title], .dot-online-offline, .qtip').tipsy({gravity: 's'});
...
var notifica = {
	...
	inPostHandle: function (r) {
		...
		if (x.length == 3 && x[0] == 0) {
			...
			$('#cseguir').html(number_format(parseInt(x[2])));
...
*/
		...
		var x = r.split('-');
		if (x.length == 2){ 
			mydialog.alert('Notificaciones', x[1]);
		}else{ 
			mydialog.close();
			var total = parseInt($('#copnu').text())+1;
			$('#copnu').text(total);
		}
	},
...
var mensaje = {
    ...
    form: function (){
         var html = '';
        if(this.vars['error']) html += '<div class="nada-gris">' + this.vars['error'] + '</div><br style="clear:both">'
//        html += '<div class="m-col1">Para:</div>'
        html += '<div class="m-col2"><input type="text" value="' + this.vars['to'] + '" maxlength="16" tabindex="0" size="20" id="msg_to" name="msg_to" placeholder="Para"/></div>'
//        html += '<div class="m-col1">Asunto:</div>'
        html += '<div class="m-col2"><input type="text" value="' + this.vars['sub'] + '" maxlength="100" tabindex="0" size="50" id="msg_subject" name="msg_subject" placeholder="Asunto"/>'
//        html += '<div class="m-col1">Mensaje:</div>'
        html += '<div class="m-col2"><textarea tabindex="0" rows="10" id="msg_body" name="msg_body" placeholder="Escribe tu mensaje...">' + this.vars['msg'] + '</textarea></div>'
        return html;                          
    },
    // FUNCIONES AUX
    ...
    alert: function(h){
      mydialog.procesando_fin();
      mydialog.alert('Aviso','<div>' + h + '</div>');  
    },
...
var shout = {
	...
	last: function () {
		...
		menu.close();
        //
		if ($('#shout_list').css('display') != 'none') {
			$('#shout_list').hide();
			$('a[name=Shout]').parent('li').removeClass('monitor-notificaciones');
		}else{
			if (($('#shout_list').css('display') == 'none') || typeof opciones.cache.last == 'undefined') {
				//$('a[name=Shout]').children('span').addClass('spinner');
				$('a[name=Shout]').parent('li').addClass('monitor-notificaciones');
				$('#shout_list').show();
				$('#welle').val('').focus();
...
var menu = {
	...
	last: function () {
		...
		if ($('#menu_list').css('display') != 'none') {
			$('#menu_list').hide();
			$('a[name=Menu]').parent('li').removeClass('monitor-notificaciones');
		}else{
			if (($('#menu_list').css('display') == 'none') || typeof menu.cache.last == 'undefined') {
				$('a[name=Menu]').parent('li').addClass('monitor-notificaciones');
				$('#menu_list').show();
				publico.stream.load('status', this);
			} else menu.show();
		}		
...
//NUEBE DE TAGS
function tags_actualiza(){
//function tags_actualiza(demo){
    $('#loading').fadeIn(250);
	$('.listags').css('opacity', 0.5);
	$.ajax({
		//type: 'POST',
		url: global_data.url + '/posts-last-tags.php',		
		cache: false,
		//data: 'demo='+demo,
		success: function(h){
			$('.listags').css('opacity', 1);
			$('.listags').html(h);
            $('#loading').fadeOut(350);
		},
		error: function(){
...
//TOP POST
function filtrar_top_post(tipo){
    $('#loading').fadeIn(250);
	$('#postops').css('opacity', 0.5);
	$.ajax({
		type: 'GET',
		url: global_data.url + '/posts-tops-posts.php',		
		cache: false,
		data: 'tipo='+tipo,
		success: function(h){
			$('#postops').css('opacity', 1);
			$('#postops').html(h);
            $('#loading').fadeOut(350);
		},
		error: function(){
...
//TOP USUARIOS
function filtrar_top_usuarios(tipo){
    $('#loading').fadeIn(250);
	$('#usutops').css('opacity', 0.5);
	$.ajax({
		type: 'GET',
		url: global_data.url + '/posts-tops-usuarios.php',		
		cache: false,
		data: 'tipo='+tipo,
		success: function(h){
			$('#usutops').css('opacity', 1);
			$('#usutops').html(h);
            $('#loading').fadeOut(350);
		},
		error: function(){
...
//POST DESTACADOS
function filtrar_post_destacados(tipo){
    $('#loading').fadeIn(250);
	$('#posdestacados').css('opacity', 0.5);
	$.ajax({
		type: 'GET',
		url: global_data.url + '/posts-destacados.php',		
		cache: false,
		data: 'tipo='+tipo,
		success: function(h){
			$('#posdestacados').css('opacity', 1);
			$('#posdestacados').html(h);
            $('#loading').fadeOut(350);
		},
		error: function(){
			$('#posdestacados').show;
...
//POST RECOMENDADOS
function filtrar_post_recomendados(tipo){
    $('#loading').fadeIn(250);
	$('#posdestacados').css('opacity', 0.5);
	$.ajax({
		type: 'GET',
		url: global_data.url + '/posts-recomendados.php',		
		cache: false,
		data: 'tipo='+tipo,
		success: function(h){
			$('#posdestacados').css('opacity', 1);
...
//MENU TOPS	SIMPLIFICADO
function pestana(parent, tab, callerid, cambio, tipo) {
	if( $('#'+tab).css('display') == 'block') return;
	$('#'+cambio).html($('.'+callerid).html());
	$('#'+parent+' > a.listo').removeClass('listo');
	$('#'+parent+' > a.'+callerid).addClass('listo');	
	$('#'+parent).hide();	
}
...
$(document).ready(function(){
	$('.mentop').hover(
		function(){
			$(this).children('.listop').show();
		},
		function(){
			$(this).children('.listop').hide();
		}
	);
});	
...
//MENU FILTRO SHOUTS HOME
function pestanas(parent, tab, callerid, cambio, tipo) {
	if( $('#'+tab).css('display') == 'block') return;
	$('#'+cambio).html($('.'+callerid).html());
	$('#'+parent+' > a.listo').removeClass('listo');
	$('#'+parent+' > a.'+callerid).addClass('listo');	
...
//FILTRO SHOUTS HOME
function filtrar_shout(tipo){
    $('#loading').fadeIn(250);
	$('#filshout').css('opacity', 0.5);
	$.ajax({
		type: 'GET',
		url: global_data.url + '/posts-shouts.php',		
		cache: false,
		data: 'tipo='+tipo,
		success: function(h){
			$('#filshout').css('opacity', 1);
			$('#filshout').html(h);
            $('#loading').fadeOut(350);
		},
		error: function(){
...
$(document).ready(function(){
    ...
    $('#masta').click(function(){
        if (a) {
            $('.ecobot').hide();
            a = false;
			$('#masta').addClass('esmes');
			$('#masta').removeClass('ismes');
			
        }
        else {
            $('.ecobot').show();
			$('#masta').removeClass('esmes');
			$('#masta').addClass('ismes');
            a = true;
        }
    });
});	
