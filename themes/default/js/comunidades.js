...
var com = {
	...
	crear_tema: function() {
		if(com.required()) {
			...
			$.ajax({
				...
				success: function(h) {
					mydialog.body(h);
					mydialog.buttons(true, true, 'Publicar', 'com.save_tema();', true, true, true, 'Cancelar', 'close', true, false);
					$('#buttons > a').removeClass('boto');
					$('#buttons > a').addClass('boto verde');
					mydialog.center();
					$.scrollTo(0, 500)
	...
	borrar:function(comid, gew){
		if(!gew){
			mydialog.show();			
			mydialog.title('Suspender Comunidad');			
			mydialog.body('&#191;Quieres suspender esta comunidad?');			
			mydialog.buttons(true, true, 'Aceptar', 'com.borrar(' + comid + ', 1)', true, false, true, 'Cancelar', 'close', true, true);			
			mydialog.center();			
		}else{
	...
	eliminar:function(comid, gew){
		if(!gew){
			mydialog.show();			
			mydialog.title('Eliminar Comunidad');			
			mydialog.body('&#191;Quieres eliminar esta comunidad?');			
			mydialog.buttons(true, true, 'Aceptar', 'com.eliminar(' + comid + ', 1)', true, false, true, 'Cancelar', 'close', true, true);			
			mydialog.center();			
		}else{
			mydialog.procesando_inicio();	  
			$('#loading').fadeIn(250);   
...
function act_comentarios(){
$('#lisco').css('opacity', 0.4);
$.ajax({
  type: 'GET',
  url: global_data.url + '/comunidades-last-comentarios.php',
  cache: false,
  success: function(h){
   $('#lisco').html(h);
  },
...
//TOP POPULARES
function filtrar_top_populares(tipo){
    $('#loading').fadeIn(250);
	$('#poputops').css('opacity', 0.5);
	$.ajax({
		type: 'GET',
		url: global_data.url + '/comunidades-populares.php',		
		cache: false,
		data: 'tipo='+tipo,
		success: function(h){
			$('#poputops').css('opacity', 1);
			$('#poputops').html(h);
            $('#loading').fadeOut(350);
		},
		error: function(){
...
//TOP VISITADOS
function filtrar_temas_vistos(tipo){
    $('#loading').fadeIn(250);
	$('#vistotops').css('opacity', 0.5);
	$.ajax({
		type: 'GET',
		url: global_data.url + '/comunidades-temas-mas-visitados.php',		
		cache: false,
		data: 'tipo='+tipo,
		success: function(h){
			$('#vistotops').css('opacity', 1);
			$('#vistotops').html(h);
            $('#loading').fadeOut(350);
		},
		error: function(){
...
function get_url(){
	...
	$.ajax({
		type: 'POST',
		url: global_data.url + '/comunidades-ncorto.php',
		data: 'nombre=' + nombre,
		success: function(h){
